// ============================================================
// EUROVIX · WERK OS — perito de diagnóstico ISTA (BMW) por IA
// ------------------------------------------------------------
// O mecânico anexa o laudo do ISTA (FOTO da tela/impressão OU PDF
// exportado) e a Claude age como um MESTRE de diagnóstico BMW:
// TRANSCREVE e traduz o laudo, extrai cada código, separa
// CAUSA-RAIZ de código CONSEQUENTE, prioriza e sugere os passos.
//
...
function instrucao(ctxTxt) {
  return [
    'Você é um MESTRE em diagnóstico BMW, anos de ISTA/ISTA-D no balcão. Recebeu o laudo de diagnóstico do ISTA de um veículo (foto da tela/impressão ou PDF)' + (ctxTxt ? ' — contexto da recepção: ' + ctxTxt : '') + '.',
    '',
    ...
    '2) FORMATO DO CÓDIGO: copie EXATAMENTE como impresso. BMW é hex (5 díg., ex. "00A6B2","801C33") ⇒ formato "hex_bmw"; SAE começa com letra+4 díg. (ex. "P0016") ⇒ "sae_p"; senão "desconhecido". NÃO converta um no outro. Caractere ambíguo em foto (0/O/D, 8/B, 5/S, 1/I, 2/Z, 6/G) ⇒ caractere_ambiguo:true e cite em observacoes, não escolha no chute.',
    ...
    '4) CAUSA-RAIZ vs CONSEQUENTE: padrão BMW — bateria fraca/subtensão ("Unterspannung", Klemme 30/15), massa/aterramento ruim, um conector ou perda de comunicação de barramento (K-CAN/PT-CAN/FlexRay, "sem mensagem do módulo X") DISPARAM dezenas de códigos em vários módulos. Marque esses como tipo "consequente"; aponte a provável causa comum em causa_raiz_provavel (SEMPRE hipótese a confirmar, nunca conclusão) e ALERTE contra trocar peça por cada código.',
...
function coerceFormato(v, codigo) {
  ...
  if (/sae|p.?code|obd/.test(sg)) return 'sae_p';
  const c = String(codigo || '').trim();
  if (/^[PBCU]\d{4}$/i.test(c)) return 'sae_p';
...
function normalizar(p, nAnexos) {
  ...
  if (!boolish(p.eh_ista, true)) {
    return { ok: true, modo: 'ia', eh_ista: false, legivel: false, recaptura_necessaria: false, motivo_recaptura: null, resumo_executivo: 'O anexo não parece um diagnóstico ISTA/BMW. Envie a tela do ISTA (memória de falhas) ou o PDF exportado.', causa_raiz_provavel: null, requer_confirmacao_profissional: false, avisos_seguranca: [], codigos: [], sistemas_afetados: [], prioridades: [], proximos_passos: [], observacoes: '', confianca: 0, veiculo: { modelo: null, chassi: null, km: null }, anexos: nAnexos };
  }
  ...
  if (temCritico && !avisos.length) avisos = ['Há código de sistema de segurança (airbag/freio/direção). Não libere o veículo — exige confirmação e medição do técnico BMW antes de qualquer ação.'];
